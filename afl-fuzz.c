...
static void usage(u8* argv0) {
  ...
  SAYF("\n%s [ options ] -- /path/to/fuzzed_app [ ... ]\n\n"
       ...
       "  -P protocol   - application protocol to be tested (e.g., RTSP, FTP, DTLS12, DNS, SMTP, SSH, TLS, MQTT, DLT)\n"
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:QN:D:W:w:P:KEq:s:RFc:")) > 0)
    ...
    switch (opt) {
      ...
      case 'P': /* protocol to be tested */
        ...
        } else if (!strcmp(optarg, "DLT")) {
          extract_requests = &extract_requests_dlt;
          extract_response_codes = &extract_response_codes_dlt;
        } else {
